      ...
      <el-table-column
        ...
        <template #default="scope">
          <el-image
              style="width: 75px; height: 50px"
              :src="scope.row.himg1"
              :preview-src-list="[scope.row.himg1]">
          </el-image>
        </template>
      </el-table-column>
      ...
      <el-dialog title="提示" v-model="dialogVisible" width="30%">
        <el-form :model="form" label-width="120px">
          <el-form-item label="名字">
            <el-input v-model="form.hname" style="width: 80%"></el-input>
          </el-form-item>
          <el-form-item label="标题">
            <el-input v-model="form.htitle" style="width: 80%"></el-input>
          </el-form-item>
          <el-form-item label="内容">
            <el-input v-model="form.hdescription" style="width: 80%"></el-input>
          </el-form-item>
...
export default {
  ...
  methods: {
    ...
    save() {
      if (this.form.id) {  // 更新
        request.put("/bigphoto", this.form).then(res => {
          console.log(res)
          if (res.code === '0') {
            this.$message({
              type: "success",
        ...
        request.post("/bigphoto", this.form).then(res => {
          console.log(res)
          if (res.code === '0') {
            this.$message({
              type: "success",
    ...
    handleDelete(id) {
      console.log(id)
      request.delete("/bigphoto/" + id).then(res => {
        if (res.code === '0') {
          this.$message({
            type: "success",
            message: "删除成功"
